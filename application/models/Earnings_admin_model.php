...
class Earnings_admin_model extends CI_Model
{
    ...
    //update bitcoin payout settings
    public function update_bitcoin_payout_settings()
    {
        $data = array(
            'payout_bitcoin_enabled' => $this->input->post('payout_bitcoin_enabled', true),
            'min_payout_bitcoin' => $this->input->post('min_payout_bitcoin', true)
        );
        $data["min_payout_bitcoin"] = get_price($data["min_payout_bitcoin"], 'database');
        $this->db->where('id', 1);
        return $this->db->update('payment_settings', $data);
    }
